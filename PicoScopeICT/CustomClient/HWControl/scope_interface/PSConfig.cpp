	...
	if (unit->handle)
	{
		for (i = 0; i < 11; i++)
		{
			...
			if (i == 3)
			{
				...
				if (strlen(reinterpret_cast<const char*>(line)) == 8 || line[3] != '4')
				{
					unit->ETS	= TRUE;
				}
				...
				// Check if MSO device
				if (strlen(reinterpret_cast<const char*>(line)) >= 7)
				{
					line[4] = toupper(line[4]);
					line[5] = toupper(line[5]);
					line[6] = toupper(line[6]);

					if(strcmp(reinterpret_cast<const char*>(line + 4), "MSO") == 0 || strcmp(reinterpret_cast<const char*>(line + 5), "MSO") == 0)
					{
						unit->digitalPorts = 2;
						unit->sigGen = SIGGEN_AWG;
					}
